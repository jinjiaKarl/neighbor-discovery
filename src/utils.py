...
ca_url = f'http://{CA_IPADDR}:{CA_PORT}'
...
# Get the distance from an access point based on signal strength
def get_distance_rssi(ap, signal_strength):
    # Use the RSSI_Localizer object to calculate the distance from the access point
    distance = localizer.getDistanceFromAP(ap, signal_strength)
    return distance['distance'] # Return the calculated distance
